...
CN = webhook-server-svc.devops.svc
[ v3_req ]
...
[alt_names]
DNS.1 = webhook-server-svc.devops.svc
EOF
...
openssl req -new -key webhook-server-tls.key -subj "/CN=webhook-server-svc.devops.svc" -config server.conf \
